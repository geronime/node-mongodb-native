 ...
 * @param {boolean} [options.customKeepAlive=false] Enable custom keep alive that does ping cmd and forcefully reinitialises a particular mongos proxy instance connection in case of connectivity troubles are detected
 * @param {number} [options.customKeepAliveTimeout=2000] Timeout for the ping command in the custom keep alive probes
 * @param {number} [options.customKeepAliveTries=5] Amount of consecutively failed/timed out pings to particular mongos proxy to trigger destroy/recreate in the server list
...
var Mongos = function(servers, options) {
  ...
  // customKeepAlive options passed down
  if(options.customKeepAlive) {
    finalOptions.customKeepAlive = true;
    finalOptions.customKeepAliveTimeout = options.customKeepAliveTimeout || 2000;
    finalOptions.customKeepAliveTries = options.customKeepAliveTries || 5;
  }
